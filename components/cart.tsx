...
import { validateCoupon } from '@/actions/order.action';
...
export default function Cart() {
  ...
  const applyCoupon = async () => {
    try {
      ...
      // Check for mock coupons first
      if (couponCode.toLowerCase() === 'save10') {
        setAppliedCoupon('SAVE10');
        setCouponDiscount(cart.totalPrice * 0.1);
        setCouponCode('');
        toast.success('Coupon applied successfully');
        return;
      } else if (couponCode.toLowerCase() === 'first20') {
        setAppliedCoupon('FIRST20');
        setCouponDiscount(cart.totalPrice * 0.2);
        setCouponCode('');
        toast.success('Coupon applied successfully');
        return;
      }

      // If not mock, validate with real API
...
function CartItem({ item, handleQuantityChange, removeFromCart }: any) {
  const [imageSrc, setImageSrc] = useState(
    getImageUrl(item.image) || '/placeholder.svg'
  );
  const [isError, setIsError] = useState(false);

  // Handle fallback for "Unknown" vendor data
  const vendorName =
    !item.vendor?.name || item.vendor.name.toLowerCase().includes('unknown')
      ? 'Vendor Unavailable'
      : item.vendor.name;

  const vendorLocation =
    !item.vendor?.location ||
    item.vendor.location.toLowerCase().includes('unknown')
      ? 'Contact support for location'
      : item.vendor.location;

  return (
    <Card className='overflow-hidden hover:shadow-xl transition-all duration-300 bg-white border border-gray-200 hover:border-[#fca311]/30'>
            ...
            <div className='space-y-3'>
              <div className='flex items-center gap-2 flex-wrap'>
                ...
                <div className='flex items-center gap-1'>
                  ...
                  <span className='text-xs text-[#fca311]'>4.8</span>
                </div>
              </div>
              ...
              <p className='text-sm text-gray-500'>{vendorLocation}</p>
              <Badge
