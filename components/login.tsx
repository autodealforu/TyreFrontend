...
export default function Login() {
  ...
  // Register OTP verification state
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Verify OTP first before completing registration
    try {
      const response = await fetch(`${apiUrl}/api/users/websites/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          otp: registerOtpCode,
                    ...
                    {!registerOtpSent ? (
                      <form onSubmit={handleRegister} className='space-y-4'>
                        ...
                        <Button
                          ...
                          disabled={isLoading}
                        >
                          {isLoading ? 'Sending Verification OTP...' : 'Send OTP & Register'}
                        </Button>
                      </form>
                    ) : (
                      <form onSubmit={handleRegister} className='space-y-4'>
                        <div className='space-y-2'>
                          <Label htmlFor='register-otp'>Enter 4-digit Verification Code</Label>
                          <Input
                            id='register-otp'
                            type='text'
                            placeholder='Enter the 4-digit OTP sent to your phone'
                            ...
                            onChange={(e) =>
                              setRegisterOtpCode(e.target.value.replace(/\D/g, ''))
                            }
                            required
                            maxLength={4}
                          />
                        ...
                        <Button
                          type='submit'
                          className='w-full'
                          disabled={isLoading || registerOtpCode.length < 4}
                        >
                          {isLoading ? 'Verifying & Creating...' : 'Verify & Complete Registration'}
                        </Button>
                      </form>
                    )}
                    <div className='text-center'>
